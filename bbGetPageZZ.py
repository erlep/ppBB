    ...
    async with async_playwright() as pw:
      ...
      if 'mapy' in url:
        ...
        browser = await pw.chromium.launch(
            headless=bbHeadLes, args=['--disable-blink-features=AutomationControlled'])
        # Don't Show the browser True / False  , slow_mo=50
        ...
        try:
          ...
          # Přidání dalších nastavení pro napodobení běžného prohlížeče
          ...
          # print(f'page: \n {page}')
          # pohyb mysi
          await page.mouse.move(100, 100)  # Pohyb na pozici (100, 100)
          # Kliknutí na pozici (100, 100)
          await page.mouse.click(100, 100, delay=100)  # Přidání zpoždění 100 ms

          # await context.storage_state(path='bbMapy.cookies.json') # ukladani cookies vypnuto 20.06.2024 17:23
          await page.goto(url, timeout=bbTimeGet)  # Wait for 10 second
          # pohyb mysi
          await page.mouse.move(100, 100)  # Pohyb na pozici (100, 100)
          # Přidání zpoždění před kliknutím
          await asyncio.sleep(1)  # Zpoždění 1 sekunda
          # Kliknutí na pozici (100, 100)
          await page.mouse.click(100, 100, delay=100)  # Přidání zpoždění 100 ms
          ...
          e = sys.exc_info()[0]
          print("Error v bbGetPage.py pro Makro. url", url, '\t\t', e)
          return ''
      else:
